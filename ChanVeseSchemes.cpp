...
field ChanVeseSchemes::ExplicitScheme(const field& phi, const double dt,  const double mu, const double nu, const double l1, const double l2) const
{
	...
	for (int i=1; i<nx_local; ++i)
	{
		for (int j=1; j<ny_local; ++j)
		{
			...
			newphi(i,j) = Phi(i,j) + dt*diracij*(mu*(firstterm+secondterm)- nu + correc);
		}
	}
...
field ChanVeseSchemes::ExplicitSchemeCPU(const field& phi, const double dt,  const double mu, const double nu, const double l1, const double l2, const int stencil)//&
//const int chargex, const int chargey, const int xmin, const int ymin)
{
	chargex = charge(me,np,phi.rows()) //TODO
	chargey = charge(me,np,phi.cols()) //TODO
	xmin = min(me,np,stencil,Nx,Ny) // TODO return xminet ymin les coordonnées du point le plus en haut et a gauche de la matrcie
	field newphi(chargex+stencil, chargey+stencil);
 c&,Cé=fctcoreection()
	newphi(:,:)=phi(me*chargex - stencil + xmin: (me+1)*chargex + stencil + min, me*chargey - stencil + ymin: (me+1)*chargey + stencil + ymin)

	for (size_t i = 0; i < stencil; i++) {
		newphi = ChanVese->ExplicitScheme(const field& newphi, const double dt,  const double mu, const double nu, const double l1, const double l2, C1,C2)
	}
	//Communication + bord
...
field ChanVeseSchemes::ExplicitSchemeGrosPhi(const field& phi, const double dt,  const double mu, const double nu, const double l1, const double l2) const
{
	const double hx(1.0), hy(1.0);
	const double eta(1e-8);
